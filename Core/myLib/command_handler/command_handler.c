...
#define SERIAL_PORT2  2  // UART2 for Modbus
...
static void Process_BasicCommands(CommandHandler_t *handler, const char* cmd) {
    if (strcmp(cmd, "status") == 0) {
        ...
        UART_HandleTypeDef* debug_uart = handler->config.command_port;
        if (debug_uart) {
        ...
        // Display uart2 (Modbus port) configuration details
        UART_HandleTypeDef* modbus_uart = handler->config.modbus_port;
        if (modbus_uart) {
            printf("=== uart2 (MODBUS) CONFIG ===\r\n");
...
static void Process_UARTCommands(CommandHandler_t *handler, const char* cmd) {
    ...
    if (strncmp(cmd, "uart1 ", 6) == 0) {
        ...
        printf("❌ Invalid UART command format. Use: uart1/uart2 <command>\r\n");
        return;
    }
    
    UART_HandleTypeDef* target_uart = (port == SERIAL_PORT1) ? handler->config.command_port : handler->config.modbus_port;
        ...
        if (strncmp(param_start, "baud ", 5) == 0) {
            ...
            if (par <= 2) {
                ...
                switch (par) {
                    case 0: // NONE
                        ...
                        handler->config.modbus_port->Init.WordLength = UART_WORDLENGTH_8B;
                        break;
                    case 1: // ODD
                        handler->config.modbus_port->Init.Parity = UART_PARITY_ODD;
                        handler->config.modbus_port->Init.WordLength = UART_WORDLENGTH_9B;
                        break;
                    case 2: // EVEN
                        handler->config.modbus_port->Init.Parity = UART_PARITY_EVEN;
                        handler->config.modbus_port->Init.WordLength = UART_WORDLENGTH_9B;
                        break;
                }
                HAL_UART_DeInit(handler->config.modbus_port);
            ...
            if (stop == 1 || stop == 2) {
                ...
                if (stop == 2) {
                    ...
                    handler->config.modbus_port->Init.StopBits = UART_STOPBITS_1;
                }
                HAL_UART_DeInit(handler->config.modbus_port);
                HAL_UART_Init(handler->config.modbus_port);
                Restart_MODBUS_DMA();
...
static void Process_ModbusCommands(CommandHandler_t *handler, const char* cmd) {
    if (strcmp(cmd, "modbus") == 0) {
        ...
        if (modbus_uart) {
            ...
            printf("💡 Standard Modbus TCP with uart config: 8N1 or 8E1 or 8O1\r\n");
        }
...
static void Show_Help(void) {
    ...
    printf("MODBUS UART CONFIG (uart2):\r\n");
